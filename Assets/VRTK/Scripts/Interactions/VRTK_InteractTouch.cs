...
#pragma warning restore 0618
        ...
        {
            Holster holster = collider.GetComponent<Holster>();
            if(holster != null)
            {
                if (holster.CheckForTouchOverride(collider.gameObject))
                {
                    return;
                }
            }
        ...
        protected virtual void OnTriggerStay(Collider collider)
        {
            ...
            if ((touchedObject == null || holster != null) || collider.transform.IsChildOf(touchedObject.transform))
            {
                triggerIsColliding = true;
            }

            if ((touchedObject == null || holster != null) && colliderInteractableObject != null && IsObjectInteractable(collider.gameObject))
            {
