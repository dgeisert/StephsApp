...
namespace VRTK
{
    ...
    public class VRTK_HeightAdjustTeleport : VRTK_BasicTeleport
    {
        ...
        protected override Vector3 GetNewPosition(Vector3 tipPosition, Transform target, bool returnOriginalPosition)
        {
            ...
            if (target.GetComponent<Ladder>() != null)
            {
                basePosition = target.GetComponent<Ladder>().top.position - new Vector3(PlayerManager.instance.head.parent.localPosition.x, 0, PlayerManager.instance.head.parent.localPosition.z);
            }
            return basePosition;
        }
