namespace VRTK.Examples
{
    ...
    public class Controller_Hand : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            if(hand == Hands.Left)
            {
                grab = PlayerManager.instance.leftManager.grab;
                use = PlayerManager.instance.leftManager.use;
            }
            else
            {
                grab = PlayerManager.instance.rightManager.grab;
                use = PlayerManager.instance.rightManager.use;
            }
            grab.GrabButtonPressed += DoGrabOn;
            grab.GrabButtonReleased += DoGrabOff;
            use.UseButtonPressed += DoUseOn;
            use.UseButtonReleased += DoUseOff;

            var handContainer = "Container";
